import { I as jsxRuntimeExports, S as reactExports, w as getDefaultExportFromCjs, a as React$1, W as requireReact, d as commonjsGlobal } from "./index.mjs";
import { A as AppShell, P as Plus, W as Wallet, S as Search, j as Trash2 } from "./AppShell-OCwEkoGu.mjs";
import { ak as useAuth, ai as supabase, D as Dialog, t as DialogTrigger, b as Button, o as DialogContent, r as DialogHeader, s as DialogTitle, L as Label$1, V as Select, Y as SelectTrigger, Z as SelectValue, W as SelectContent, X as SelectItem, n as CreditCard, C as Card, ab as formatDate, g as Check, $ as X, aj as toast, a8 as createLucideIcon, ag as reactDomExports, a2 as clsx } from "./router-Bktayy9l.mjs";
...
import { r as requireShim } from "./index-CxZfSfQO.mjs";
import "node:events";
...
const CATEGORIES = ["alimentação", "transporte", "moradia", "saúde", "educação", "lazer", "salário", "freelance", "investimento", "outros"];
...
const PIE_COLORS = ["#0891b2", "#059669", "#d97706", "#dc2626", "#7c3aed", "#db2777", "#1e3a8a", "#475569", "#f59e0b", "#10b981"];
...
function FinancesPage() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AppShell, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(FinancesContent, {}) });
}
function FinancesContent() {
  ...
  const [editingInlineId, setEditingInlineId] = reactExports.useState(null);
  const [title, setTitle] = reactExports.useState("");
  const [amount, setAmount] = reactExports.useState("");
  const [type, setType] = reactExports.useState("despesa");
  const [category, setCategory] = reactExports.useState("outros");
  const [date2, setDate] = reactExports.useState((/* @__PURE__ */ new Date()).toISOString().split("T")[0]);
  const [notes, setNotes] = reactExports.useState("");
  const [recurring, setRecurring] = reactExports.useState("none");
  const [dueDate, setDueDate] = reactExports.useState("");
  const [isCreditCard, setIsCreditCard] = reactExports.useState(false);
  const [installments, setInstallments] = reactExports.useState("1");
  const [reportMode, setReportMode] = reactExports.useState("month");
  const [reportMonth, setReportMonth] = reactExports.useState(getDefaultMonth);
  const [reportFrom, setReportFrom] = reactExports.useState("");
  const [reportTo, setReportTo] = reactExports.useState("");
  const [inlineTitle, setInlineTitle] = reactExports.useState("");
  const [inlineAmount, setInlineAmount] = reactExports.useState("");
  const [inlineCategory, setInlineCategory] = reactExports.useState("");
  const [inlineDate, setInlineDate] = reactExports.useState("");
  const load = reactExports.useCallback(async () => {
    if (!user) return;
    const {
      data
    } = await supabase.from("personal_finances").select("*").eq("user_id", user.id).order("date", {
      ascending: false
    });
    setItems(data || []);
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      const {
        error
      } = await supabase.from("personal_finances").update(payload).eq("id", editing.id);
      if (error) {
      ...
      if (isCreditCard && parseInt(installments) > 1) {
        ...
        const {
          data: parent,
          error
        } = await supabase.from("personal_finances").insert({
          user_id: user.id,
          title: `${title.trim()} (1/${totalInstallments})`,
        ...
        for (let i = 2; i <= totalInstallments; i++) {
          const installDate = addMonths(date2, i - 1);
          childRows.push({
            user_id: user.id,
            ...
            notes: notes.trim() || null,
            recurring: "none",
            due_date: dueDate ? addMonths(dueDate, i - 1) : null,
            is_credit_card: true,
            installments: totalInstallments,
        ...
        if (childRows.length > 0) {
          const {
            error: e22
          } = await supabase.from("personal_finances").insert(childRows);
          if (e22) {
            toast.error(e22.message);
            return;
          }
        }
        ...
        const parentRes = await supabase.from("personal_finances").insert({
          ...
          installments: occurrences,
          installment_number: 1,
          paid: false
        }).select("id").single();
        if (parentRes.error) {
          toast.error(parentRes.error.message);
          return;
        }
        const childRows = [];
        for (let i = 2; i <= occurrences; i++) {
          const nextDate = recurring === "monthly" ? addMonths(date2, i - 1) : addWeeks(date2, i - 1);
          childRows.push({
            user_id: user.id,
            title: title.trim(),
            amount: amountNum,
  ...
  const togglePaid = async (f) => {
    await supabase.from("personal_finances").update({
      paid: !f.paid
    }).eq("id", f.id);
    load();
  };
  const startInlineEdit = (f) => {
  ...
  const totalReceita = items.filter((f) => f.type === "receita").reduce((s2, f) => s2 + Number(f.amount), 0);
  const totalDespesa = items.filter((f) => f.type === "despesa").reduce((s2, f) => s2 + Number(f.amount), 0);
  const saldo = totalReceita - totalDespesa;
  const currentMonth = getDefaultMonth();
  const ccInvoice = reactExports.useMemo(() => {
    return items.filter((f) => f.is_credit_card && !f.paid && f.date.startsWith(currentMonth)).reduce((s2, f) => s2 + Number(f.amount), 0);
  }, [items, currentMonth]);
  const dueSoon = reactExports.useMemo(() => {
  ...
  const reportItems = reactExports.useMemo(() => {
  ...
  const monthlyData = reactExports.useMemo(() => {
    ...
    reportItems.forEach((f) => {
      ...
      if (f.type === "receita") map2[m].receita += Number(f.amount);
      else map2[m].despesa += Number(f.amount);
    });
    return Object.values(map2).sort((a2, b) => a2.month.localeCompare(b.month)).slice(-12).map((d) => ({
      ...d,
      saldo: d.receita - d.despesa,
      label: (/* @__PURE__ */ new Date(d.month + "-15")).toLocaleDateString("pt-BR", {
        month: "short",
        year: "2-digit"
      })
    }));
  ...
  const categoryData = reactExports.useMemo(() => {
    ...
    reportItems.filter((f) => f.type === "despesa").forEach((f) => {
      map2[f.category] = (map2[f.category] || 0) + Number(f.amount);
    });
    return Object.entries(map2).map(([name, value]) => ({
      name,
      value
    })).sort((a2, b) => b.value - a2.value);
  }, [reportItems]);
  const categoryIncomeData = reactExports.useMemo(() => {
    const map2 = {};
  ...
  const reportTotalReceita = reportItems.filter((f) => f.type === "receita").reduce((s2, f) => s2 + Number(f.amount), 0);
  const reportTotalDespesa = reportItems.filter((f) => f.type === "despesa").reduce((s2, f) => s2 + Number(f.amount), 0);
  ...
  const exportPDF = async () => {
    ...
    const periodLabel = reportMode === "month" ? (/* @__PURE__ */ new Date(reportMonth + "-15")).toLocaleDateString("pt-BR", {
      month: "long",
      year: "numeric"
    }) : `${reportFrom || "início"} a ${reportTo || "hoje"}`;
    let y2 = await addBrandedHeader(doc, "Relatório de Finanças Pessoais", `Período: ${periodLabel}`, `Gerado em: ${(/* @__PURE__ */ new Date()).toLocaleDateString("pt-BR")}`);
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text("Resumo Financeiro", 14, y2);
    y2 += 10;
    const boxW = (pw - 42) / 3;
    doc.setFillColor(236, 253, 245);
    doc.roundedRect(14, y2, boxW, 22, 3, 3, "F");
    doc.setTextColor(5, 150, 105);
    doc.setFontSize(9);
    doc.text("Receitas", 18, y2 + 8);
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text(fmt(reportTotalReceita), 18, y2 + 17);
    doc.setFillColor(254, 242, 242);
    doc.roundedRect(14 + boxW + 7, y2, boxW, 22, 3, 3, "F");
    doc.setTextColor(220, 38, 38);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text("Despesas", 18 + boxW + 7, y2 + 8);
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text(fmt(reportTotalDespesa), 18 + boxW + 7, y2 + 17);
    const balColor = reportSaldo >= 0 ? [236, 253, 245] : [254, 242, 242];
    doc.setFillColor(balColor[0], balColor[1], balColor[2]);
    doc.roundedRect(14 + (boxW + 7) * 2, y2, boxW, 22, 3, 3, "F");
    doc.setTextColor(reportSaldo >= 0 ? 5 : 220, reportSaldo >= 0 ? 150 : 38, reportSaldo >= 0 ? 105 : 38);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text("Saldo", 18 + (boxW + 7) * 2, y2 + 8);
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text(fmt(reportSaldo), 18 + (boxW + 7) * 2, y2 + 17);
    y2 += 30;
    if (categoryData.length > 0) {
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("Top Categorias de Despesa", 14, y2);
      y2 += 6;
      const top5 = categoryData.slice(0, 5);
      const maxVal = top5[0]?.value || 1;
      const barMaxW = pw - 80;
      top5.forEach((c2, i) => {
        const pct = c2.value / maxVal;
        doc.setFillColor(240, 245, 255);
        doc.rect(14, y2, barMaxW + 40, 8, "F");
        doc.setFillColor(30, 58, 138);
        doc.rect(14, y2, pct * barMaxW, 8, "F");
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(7);
        doc.setFont("helvetica", "bold");
        if (pct > 0.15) doc.text(c2.name, 16, y2 + 5.5);
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(8);
        doc.setFont("helvetica", "normal");
        doc.text(fmt(c2.value), pw - 14, y2 + 5.5, {
          align: "right"
        });
        y2 += 10;
      });
    ...
    if (monthlyData.length > 1) {
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("Evolução Mensal do Saldo", 14, y2);
      y2 += 2;
      autoTable(doc, {
        startY: y2,
      ...
      y2 = doc.lastAutoTable?.finalY + 8 || y2 + 30;
    }
    if (y2 > 220) {
      doc.addPage();
      y2 = 20;
    ...
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.text("Lançamentos Detalhados", 14, y2);
    y2 += 2;
    const rows = reportItems.map((f) => [f.date, f.title, f.category, f.type === "receita" ? "Receita" : "Despesa", f.is_credit_card ? `Cartão ${f.installment_number}/${f.installments}` : "-", fmt(Number(f.amount))]);
    autoTable(doc, {
      startY: y2,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-6 lg:p-10 space-y-8 max-w-6xl mx-auto", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "flex flex-col sm:flex-row sm:items-end justify-between gap-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "font-display text-3xl lg:text-4xl font-bold mt-1", children: "Finanças" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground mt-1", children: "Controle suas receitas e despesas pessoais." })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(Dialog, { open, onOpenChange: (v) => {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { className: "max-h-[90vh] overflow-y-auto", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(DialogHeader, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(DialogTitle, { children: editing ? "Editar registro" : "Novo registro" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4 mt-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { value: title, onChange: (e) => setTitle(e.target.value), placeholder: "Ex: Almoço, Salário..." })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-4", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { children: "Valor (R$)" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "number", step: "0.01", min: "0", value: amount, onChange: (e) => setAmount(e.target.value), placeholder: "0,00" })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { children: "Data" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "date", value: date2, onChange: (e) => setDate(e.target.value) })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-4", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: type, onValueChange: (v) => {
                  setType(v);
                  if (v === "receita") setIsCreditCard(false);
                }, children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { children: "Categoria" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: category, onValueChange: setCategory, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, {}) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(SelectContent, { children: CATEGORIES.map((c2) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: c2, className: "capitalize", children: c2 }, c2)) })
                ] })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-4", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { children: "Recorrência" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: recurring, onValueChange: setRecurring, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, {}) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(SelectContent, { children: RECURRING_OPTIONS.map((o) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: o.value, children: o.label }, o.value)) })
                ] })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { children: "Vencimento" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "date", value: dueDate, onChange: (e) => setDueDate(e.target.value) })
              ] })
            ] }),
            type === "despesa" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3 rounded-lg border p-3 bg-muted/30", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Checkbox, { checked: isCreditCard, onCheckedChange: (v) => setIsCreditCard(!!v), id: "cc" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { htmlFor: "cc", className: "text-sm font-medium flex items-center gap-1.5 cursor-pointer", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(CreditCard, { className: "h-4 w-4" }),
                  " Cartão de Crédito"
                ] })
              ] }),
              isCreditCard && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-xs", children: "Parcelas" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "number", min: "1", max: "48", value: installments, onChange: (e) => setInstallments(e.target.value), placeholder: "1" }),
                parseInt(installments) > 1 && amount && /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs text-muted-foreground", children: [
                  installments,
                  "x de ",
                  fmt(Math.round(parseFloat(amount) / parseInt(installments) * 100) / 100)
                ] })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { children: "Observações" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Textarea, { value: notes, onChange: (e) => setNotes(e.target.value), rows: 2 })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { onClick: handleSave, className: "w-full bg-gradient-primary text-primary-foreground", children: editing ? "Salvar alterações" : "Adicionar" })
          ] })
        ] })
      ] })
    ] }),
    dueSoon.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-4 border-l-4 border-l-warning bg-warning/5 shadow-card flex items-start gap-3", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(TriangleAlert, { className: "h-5 w-5 text-warning shrink-0 mt-0.5" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-semibold text-sm", children: "Contas vencendo em breve" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground space-y-0.5 mt-1", children: dueSoon.map((f) => /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { children: [
          f.title,
          " — vence ",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-5 shadow-card border-0 relative overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute top-0 right-0 h-20 w-20 rounded-full bg-gradient-to-br from-success/15 to-success/5 blur-2xl" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex items-center gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex h-10 w-10 items-center justify-center rounded-xl bg-success/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ArrowUpRight, { className: "h-5 w-5 text-success" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "Receitas" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xl font-display font-bold text-success", children: fmt(totalReceita) })
          ] })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-5 shadow-card border-0 relative overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute top-0 right-0 h-20 w-20 rounded-full bg-gradient-to-br from-destructive/15 to-destructive/5 blur-2xl" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex items-center gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex h-10 w-10 items-center justify-center rounded-xl bg-destructive/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ArrowDownRight, { className: "h-5 w-5 text-destructive" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "Despesas" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xl font-display font-bold text-destructive", children: fmt(totalDespesa) })
          ] })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-5 shadow-card border-0 relative overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute top-0 right-0 h-20 w-20 rounded-full bg-gradient-to-br from-accent/15 to-accent/5 blur-2xl" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex items-center gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex h-10 w-10 items-center justify-center rounded-xl bg-accent/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Wallet, { className: "h-5 w-5 text-accent" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "Saldo" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `text-xl font-display font-bold ${saldo >= 0 ? "text-success" : "text-destructive"}`, children: fmt(saldo) })
          ] })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-5 shadow-card border-0 relative overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute top-0 right-0 h-20 w-20 rounded-full bg-gradient-to-br from-purple-500/15 to-purple-500/5 blur-2xl" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex items-center gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex h-10 w-10 items-center justify-center rounded-xl bg-purple-500/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(CreditCard, { className: "h-5 w-5 text-purple-500" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "Fatura Cartão (mês)" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xl font-display font-bold text-purple-500", children: fmt(ccInvoice) })
          ] })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(Tabs, { defaultValue: "list", className: "space-y-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsList, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsTrigger, { value: "list", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { className: "h-4 w-4 mr-1.5" }),
          "Lista"
        ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsContent, { value: "list", className: "space-y-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-wrap gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex-1 min-w-[200px]", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { className: "absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { value: search, onChange: (e) => setSearch(e.target.value), placeholder: "Buscar...", className: "pl-10" })
          ] }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: filterCat, onValueChange: setFilterCat, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { className: "w-[160px]", children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, { placeholder: "Categoria" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(SelectContent, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: "all", children: "Todas categorias" }),
              CATEGORIES.map((c2) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: c2, className: "capitalize", children: c2 }, c2))
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-card overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full text-sm", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "border-b bg-muted/30", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-semibold", children: "Descrição" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-semibold", children: "Categoria" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-semibold", children: "Data" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-semibold", children: "Info" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-right px-4 py-3 font-semibold", children: "Valor" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-right px-4 py-3 font-semibold w-28", children: "Ações" })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("tbody", { className: "divide-y", children: [
            filtered.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { children: /* @__PURE__ */ jsxRuntimeExports.jsx("td", { colSpan: 6, className: "text-center py-12 text-muted-foreground", children: "Nenhum registro encontrado." }) }),
            filtered.map((f) => /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { className: `hover:bg-muted/30 transition ${f.paid ? "opacity-50" : ""}`, children: editingInlineId === f.id ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { value: inlineTitle, onChange: (e) => setInlineTitle(e.target.value), className: "h-8 text-sm" }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: inlineCategory, onValueChange: setInlineCategory, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { className: "h-8 text-xs", children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, {}) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(SelectContent, { children: CATEGORIES.map((c2) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: c2, className: "capitalize", children: c2 }, c2)) })
              ] }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "date", value: inlineDate, onChange: (e) => setInlineDate(e.target.value), className: "h-8 text-sm" }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-2" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-2 text-right", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "number", step: "0.01", value: inlineAmount, onChange: (e) => setInlineAmount(e.target.value), className: "h-8 text-sm w-24 ml-auto" }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-2 text-right", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-end gap-1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => saveInlineEdit(f.id), className: "p-1.5 rounded hover:bg-success/10 text-success", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { className: "h-3.5 w-3.5" }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => setEditingInlineId(null), className: "p-1.5 rounded hover:bg-muted", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { className: "h-3.5 w-3.5" }) })
              ] }) })
            ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                f.type === "receita" ? /* @__PURE__ */ jsxRuntimeExports.jsx(TrendingUp, { className: "h-4 w-4 text-success shrink-0" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(TrendingDown, { className: "h-4 w-4 text-destructive shrink-0" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-medium", children: f.title })
              ] }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 capitalize text-muted-foreground", children: f.category }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-muted-foreground", children: formatDate(f.date) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5 text-xs text-muted-foreground", children: [
                f.is_credit_card && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-500", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(CreditCard, { className: "h-3 w-3" }),
                  f.installment_number,
                  "/",
                  f.installments
                ] }),
                f.recurring !== "none" && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-accent/10 text-accent", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(Repeat, { className: "h-3 w-3" }),
                  f.recurring
                ] }),
                f.due_date && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-warning", children: formatDate(f.due_date) })
              ] }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: `px-4 py-3 text-right font-semibold ${f.type === "receita" ? "text-success" : "text-destructive"}`, children: [
                f.type === "receita" ? "+" : "-",
                fmt(Number(f.amount))
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-right", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-end gap-0.5", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => togglePaid(f), className: `p-1.5 rounded ${f.paid ? "text-success" : "hover:bg-muted"}`, title: f.paid ? "Marcar não pago" : "Marcar pago", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { className: "h-3.5 w-3.5" }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => startInlineEdit(f), className: "p-1.5 rounded hover:bg-muted", title: "Editar rápido", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Pencil, { className: "h-3.5 w-3.5" }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => handleDelete(f.id), className: "p-1.5 rounded hover:bg-destructive/10 text-destructive", title: "Excluir", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "h-3.5 w-3.5" }) })
              ] }) })
            ] }) }, f.id))
          ] })
        ] }) }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsContent, { value: "report", className: "space-y-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "p-4 shadow-card border-0", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-wrap items-end gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1.5", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-xs", children: "Filtro" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: reportMode, onValueChange: (v) => setReportMode(v), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { className: "w-[130px] h-9", children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, {}) }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs(SelectContent, { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: "month", children: "Mês" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: "range", children: "Intervalo" })
              ] })
            ] })
          ] }),
          reportMode === "month" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1.5", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-xs", children: "Mês" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "month", value: reportMonth, onChange: (e) => setReportMonth(e.target.value), className: "w-[180px] h-9" })
          ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1.5", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-xs", children: "De" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "date", value: reportFrom, onChange: (e) => setReportFrom(e.target.value), className: "w-[160px] h-9" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1.5", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-xs", children: "Até" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Input, { type: "date", value: reportTo, onChange: (e) => setReportTo(e.target.value), className: "w-[160px] h-9" })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ml-auto flex items-center gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-sm text-muted-foreground", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-success font-semibold", children: fmt(reportTotalReceita) }),
              " · ",
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-destructive font-semibold", children: fmt(reportTotalDespesa) }),
              " · Saldo ",
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: reportSaldo >= 0 ? "text-success font-semibold" : "text-destructive font-semibold", children: fmt(reportSaldo) })
            ] }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-6 shadow-card", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display font-bold text-lg mb-4", children: "Receitas vs Despesas por Mês" }),
          monthlyData.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm text-center py-8", children: "Sem dados." }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 300, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(BarChart, { data: monthlyData, children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "receita", name: "Receitas", fill: "#059669", radius: [4, 4, 0, 0] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "despesa", name: "Despesas", fill: "#dc2626", radius: [4, 4, 0, 0] })
          ] }) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-6 shadow-card", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display font-bold text-lg mb-4", children: "Evolução do Saldo" }),
          monthlyData.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm text-center py-8", children: "Sem dados." }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 250, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(LineChart, { data: monthlyData, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "var(--border)" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(XAxis, { dataKey: "label", tick: {
              fontSize: 12
            } }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { formatter: (v) => fmt(Number(v)) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Line, { type: "monotone", dataKey: "saldo", name: "Saldo", stroke: "#0891b2", strokeWidth: 2, dot: {
              r: 4
            } })
          ] }) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-6", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-6 shadow-card", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display font-bold text-lg mb-4", children: "Despesas por Categoria" }),
            categoryData.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm text-center py-8", children: "Sem despesas." }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 280, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(PieChart, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Pie, { data: categoryData, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", outerRadius: 90, label: (p) => `${p.name ?? ""} ${((p.percent ?? 0) * 100).toFixed(0)}%`, labelLine: false, children: categoryData.map((_, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: PIE_COLORS[i % PIE_COLORS.length] }, i)) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { formatter: (v) => fmt(Number(v)) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Legend, {})
            ] }) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "p-6 shadow-card", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display font-bold text-lg mb-4", children: "Receitas por Categoria" }),
            categoryIncomeData.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm text-center py-8", children: "Sem receitas." }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 280, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(PieChart, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Pie, { data: categoryIncomeData, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", outerRadius: 90, label: (p) => `${p.name ?? ""} ${((p.percent ?? 0) * 100).toFixed(0)}%`, labelLine: false, children: categoryIncomeData.map((_, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: PIE_COLORS[i % PIE_COLORS.length] }, i)) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { formatter: (v) => fmt(Number(v)) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Legend, {})
            ] }) })
          ] })
