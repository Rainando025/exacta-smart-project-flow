    ...
    <div className="p-6 lg:p-10 max-w-4xl mx-auto space-y-6">
      ...
      <div className="space-y-4">
        ...
        {items.map((a) => {
          ...
          return (
            <Card key={a.id} className={`p-5 shadow-card group relative ${a.pinned ? "border-l-4 border-l-accent" : ""}`}>
              {a.pinned && <Pin className="absolute top-3 right-3 h-4 w-4 text-accent fill-accent" />}
              <div className="flex items-start gap-3">
                <div className="h-10 w-10 rounded-full bg-gradient-accent flex items-center justify-center text-accent-foreground font-bold text-sm">
                ...
                {a.author_id === user?.id && (
                  <button onClick={() => remove(a.id)} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive transition">
